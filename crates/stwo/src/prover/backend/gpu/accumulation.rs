...
impl AccumulationOps for GpuBackend {
    ...
    fn lift_and_accumulate(
        ...
        // Delegate to SimdBackend via transmute since GpuBackend columns are
        // layout-compatible with SimdBackend columns.
        let simd_cols: Vec<SecureColumnByCoords<SimdBackend>> = cols
            .into_iter()
            .map(|col| unsafe { std::mem::transmute(col) })
            .collect();
        let result = SimdBackend::lift_and_accumulate(simd_cols);
        result.map(|col| unsafe { std::mem::transmute(col) })
    }
}
