...
// OBELYSK: Import core FRI functions to eliminate code duplication
use crate::core::fri::{fold_circle_into_line as core_fold_circle_into_line, fold_line as core_fold_line};
...
/// OBELYSK: Refactored to delegate to core::fri::fold_line, eliminating code duplication.
/// This wrapper handles the type conversion between LineEvaluation and raw slices.
pub fn fold_line_cpu(
    ...
    // Collect values to slice for core function
    let values_vec: Vec<_> = eval.values.into_iter().collect();
    
    // Delegate to core implementation - single source of truth
    let (new_domain, folded_values) = core_fold_line(&values_vec, domain, alpha);
    
    // Convert Vec<SecureField> back to SecureColumnByCoords
    let folded_column: SecureColumnByCoords<CpuBackend> = folded_values.into_iter().collect();
    
    LineEvaluation::new(new_domain, folded_column)
}

/// OBELYSK: Refactored to delegate to core::fri::fold_circle_into_line, eliminating code duplication.
/// This wrapper handles the type conversion between SecureEvaluation/LineEvaluation and raw slices.
pub fn fold_circle_into_line_cpu(
    ...
    // Collect source values to slice
    let src_values: Vec<_> = src.values.into_iter().collect();
    
    // Collect destination values to mutable slice
    let mut dst_values: Vec<_> = dst.values.into_iter().collect();
    
    // Delegate to core implementation - single source of truth
    core_fold_circle_into_line(&mut dst_values, &src_values, domain, alpha);
    
    // Write back the folded values
    for (i, val) in dst_values.into_iter().enumerate() {
        dst.values.set(i, val);
    }
}
