...
impl FromIterator<BaseField> for BaseColumn {
    fn from_iter<I: IntoIterator<Item = BaseField>>(iter: I) -> Self {
        // Collect into Vec first since array_chunks is unstable for iterators
        let values: Vec<BaseField> = iter.into_iter().collect();
        let mut data = Vec::new();
        let mut length = 0;
        
        // Process full chunks
        for chunk in values.chunks_exact(N_LANES) {
            let arr: [BaseField; N_LANES] = chunk.try_into().unwrap();
            data.push(PackedBaseField::from_array(arr));
            length += N_LANES;
        }
        
        // Handle remainder
        let remainder = values.chunks_exact(N_LANES).remainder();
...
impl FromIterator<CM31> for CM31Column {
    fn from_iter<I: IntoIterator<Item = CM31>>(iter: I) -> Self {
        ...
        // Process full chunks
        for chunk in values.chunks_exact(N_LANES) {
            let arr: [CM31; N_LANES] = chunk.try_into().unwrap();
            data.push(PackedCM31::from_array(arr));
            length += N_LANES;
        }
...
impl FromIterator<SecureField> for SecureColumn {
    fn from_iter<I: IntoIterator<Item = SecureField>>(iter: I) -> Self {
        ...
        // Process full chunks
        for chunk in values.chunks_exact(N_LANES) {
            let arr: [SecureField; N_LANES] = chunk.try_into().unwrap();
            data.push(PackedSecureField::from_array(arr));
            length += N_LANES;
        }
