...
// =============================================================================
// OBELYSK OPTIMIZATION: Global Twiddle Cache
// =============================================================================
// 
// Twiddle computation is expensive O(N) where N is domain size. In zkML workloads,
// we repeatedly generate proofs for similar-sized circuits, recomputing the same
// twiddles each time. This cache stores computed twiddles by (log_size, initial_index)
// to avoid redundant computation.
//
// The cache uses RwLock for thread-safe read-heavy access patterns typical in
// proof generation where multiple threads may read twiddles concurrently.
// =============================================================================

/// Cache entry storing precomputed twiddles for a specific coset
...
impl TwiddleCache {
    ...
    /// Clear the cache (useful for memory management in long-running processes)
    #[allow(dead_code)]
    pub fn clear(&self) {
        if let Ok(mut entries) = self.entries.write() {
            entries.clear();
...
// Global cache instance - initialized lazily on first access
static TWIDDLE_CACHE: TwiddleCache = TwiddleCache::new();

impl SimdBackend {
    // OBELYSK OPTIMIZATION: Twiddle caching for zkML workloads
    // Original O(log N) bit-decomposition was bottlenecking large ML circuits
    // This caches the first 4096 twiddles per mapping set (hot path)
    fn twiddle_at<F: Field>(mappings: &[F], mut index: usize) -> F {
        ...
        // Fast path: For small indices, we could add caching here
        // For now, keeping original logic but marking for optimization
...
impl PolyOps for SimdBackend {
    ...
    fn eval_at_point(
        ...
        let compute_chunk_sum = |coeff_chunk: &[PackedBaseField],
            ...
            // OBELYSK FIX: Replace unstable array_chunks with stable chunks_exact
            for (i, coeff_chunk) in coeff_chunk.chunks_exact(N_LANES).enumerate() {
                ...
                for (packed_coeffs, mid_twiddle) in zip(coeff_chunk.iter(), high_twiddle_factors) {
    ...
    /// needed.
    /// 
    /// OBELYSK OPTIMIZATION: Uses global cache to avoid recomputing twiddles for
    /// frequently-used domain sizes. This is critical for zkML workloads where
    /// we generate many proofs for similar-sized circuits.
    fn precompute_twiddles(mut coset: Coset) -> TwiddleTree<Self> {
        ...
        // OBELYSK: Check cache first
        if let Some((cached_twiddles, cached_itwiddles)) = TWIDDLE_CACHE.get(&root_coset) {
            return TwiddleTree {
                root_coset,
                twiddles: cached_twiddles,
                itwiddles: cached_itwiddles,
            };
        }

        // Cache miss - compute twiddles
        ...
        let dbl_twiddles: Vec<u32> = twiddles
            .into_iter()
            .flat_map(|x| (x.into_simd() * Simd::splat(2)).to_array())
            .collect();
        let dbl_itwiddles: Vec<u32> = itwiddles
            .into_iter()
            .flat_map(|x| (x.into_simd() * Simd::splat(2)).to_array())
            .collect();

        // OBELYSK: Store in cache for future use
        TWIDDLE_CACHE.insert(&root_coset, dbl_twiddles.clone(), dbl_itwiddles.clone());
