...
fn accumulate_quotients_on_subdomain(
    ...
    // SAFETY: All elements are written via set_packed in the accumulate closure below
    // before any reads occur. The iterator processes all subdomain.size() elements.
    // OBELYSK: Verified that the loop covers all indices through chunks_exact(4).
    let mut values =
    ...
    // Extend the evaluation to the full domain.
    // SAFETY: All elements are written via copy_from_slice in the caller's loop
    // which iterates over subdomain_shifts and fills all domain.size() elements.
    // OBELYSK: Verified that the loop covers all indices through subdomain_shifts iteration.
    let extended_eval =
