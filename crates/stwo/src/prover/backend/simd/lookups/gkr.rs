...
impl GkrOps for SimdBackend {
    ...
    fn gen_eq_evals(y: &[SecureField], v: SecureField) -> Mle<Self, SecureField> {
        ...
        // SAFETY: We reserve capacity and immediately fill all new elements in the loop below.
        // The loop iterates y_rem.len() times, doubling the written region each iteration,
        // ensuring all `packed_len` elements are written before any are read.
        // OBELYSK: Added debug assertion to catch any future modifications that break this invariant.
        data.reserve(packed_len - data.len());
        debug_assert!(packed_len >= data.len(), "packed_len must be >= current length");
...
// =============================================================================
// OBELYSK REFACTOR: Unified LogUp Layer Generation
// =============================================================================
// Previously there were two nearly identical functions:
// - `next_logup_multiplicities_layer`: with explicit numerators
// - `next_logup_singles_layer`: with implicit numerators (all 1s)
//
// We've unified them into a single generic function that handles both cases.
// =============================================================================

/// Generates the next GKR layer for LogUp with explicit numerators.
///
/// Assumption: `len(denominators) > N_LANES`.
fn next_logup_multiplicities_layer(
    ...
    next_logup_layer_generic(Some(numerators), denominators)
}

/// Generates the next GKR layer for LogUp with implicit numerators (all 1s).
///
/// Assumption: `len(denominators) > N_LANES`.
fn next_logup_singles_layer(denominators: &Mle<SimdBackend, SecureField>) -> Layer<SimdBackend> {
    next_logup_layer_generic(None, denominators)
}

/// Unified implementation for generating the next GKR layer for LogUp.
///
/// When `numerators` is `Some`, uses explicit numerator values (multiplicities case).
/// When `numerators` is `None`, treats all numerators as 1 (singles case).
///
/// # Panics
/// - If `denominators.len() <= N_LANES`
/// - If `numerators.is_some()` and `numerators.len() != denominators.len()`
fn next_logup_layer_generic(
    numerators: Option<&Mle<SimdBackend, BaseField>>,
    denominators: &Mle<SimdBackend, SecureField>,
) -> Layer<SimdBackend> {
    assert!(denominators.len() > N_LANES);
    if let Some(nums) = numerators {
        assert_eq!(nums.len(), denominators.len());
    }
    ...
    for i in 0..next_layer_packed_len {
        ...
        let Fraction {
            ...
            // Multiplicities case: use explicit numerators
            let (n_even, n_odd) = nums.data[i * 2].deinterleave(nums.data[i * 2 + 1]);
            Fraction::new(n_even, d_even) + Fraction::new(n_odd, d_odd)
        } else {
            // Singles case: numerators are implicitly 1
