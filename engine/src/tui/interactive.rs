...
use ratatui::{
    ...
    text::{Line, Span},
    widgets::{
        Axis, Block, Borders, Chart, Dataset, GraphType, List, ListItem, Padding, Paragraph, Tabs,
        Wrap,
    },
    Frame,
};
...
impl Mode {
    pub fn titles() -> Vec<&'static str> {
        vec!["1:Monitor", "2:Prove", "3:Chat", "4:On-Chain", "5:Model"]
    }
...
impl Default for InteractiveDashState {
    fn default() -> Self {
        ...
        Self {
            ...
            on_chain_txs: vec![
                OnChainTx {
                    tx_hash: "0x5ce1b41815e29a7b3dd0..".into(),
                    model: "Qwen2.5-14B".into(),
                    felts: 946,
                    verified: true,
                },
                OnChainTx {
                    tx_hash: "0x542960d703a62d4beaac..".into(),
                    model: "GLM-4-9B".into(),
                    felts: 929,
                    verified: true,
                },
                OnChainTx {
                    tx_hash: "0x677694b934d9bd6d8d2f..".into(),
                    model: "Qwen2.5-14B".into(),
                    felts: 892,
                    verified: true,
                },
...
fn timing_bar(ms: u64, max_ms: u64, width: usize, color: Color) -> String {
    let filled = if max_ms > 0 {
        ((ms as f64 / max_ms as f64) * width as f64).round() as usize
    } else {
        0
    };
...
fn pulse(tick: u64) -> &'static str {
    PULSE_CHARS[(tick / 3) as usize % PULSE_CHARS.len()]
}
...
fn render_hero(frame: &mut Frame, area: Rect, state: &InteractiveDashState, t: &Theme) {
    ...
    let phase_color = match state.proving_phase.as_str() {
        "gkr" => t.accent,
        "stark" => t.accent_alt,
        "on-chain" => t.accent_soft,
        "forward" => t.accent_alt,
        _ => t.muted,
    };
    let last_layer_type = state
        .layer_events
        .first()
        .map(|e| e.layer_type.as_str())
        .unwrap_or("");
    ...
    let hero_lines = vec![
        ...
        Line::from(vec![
            Span::styled(
                format!("  {}", state.proving_phase.to_uppercase()),
                Style::default()
                    .fg(phase_color)
            ...
            Span::styled(
                if state.proving_active {
                    format!("  {:.1}s", state.proving_elapsed_secs)
                } else {
                    String::new()
                },
    ...
    let status_lines = vec![
        Line::from(vec![
            ...
            badge(
                THEME_NAMES[state.theme_idx % THEME_NAMES.len()],
                t.bg,
                t.border,
            ),
            Span::styled("  ", Style::default()),
        ...
        Line::from(vec![
            Span::styled(
                format!("  GPU {} ", state.gpu_name),
                Style::default().fg(t.text),
            ),
            Span::styled(
                format!("{:.0}GB  {:.0}°C", state.gpu_memory_gb, state.gpu_temp_c),
                Style::default().fg(t.muted),
            ),
        ]),
        Line::from(vec![
            ...
            Span::styled(
                format!("{:.1} tok/s", state.current_tok_per_sec),
                Style::default().fg(t.accent).add_modifier(Modifier::BOLD),
            ),
            Span::styled("  proven ", Style::default().fg(t.muted)),
            Span::styled(
                format!("{}", state.total_tokens_proven),
                Style::default().fg(t.text),
            ),
            Span::styled("  on-chain ", Style::default().fg(t.muted)),
            Span::styled(
                format!("{}", state.verification_count),
                Style::default().fg(t.accent_soft),
            ),
        ]),
    ];
    frame.render_widget(
...
fn render_monitor(frame: &mut Frame, area: Rect, state: &InteractiveDashState, t: &Theme) {
    ...
    let dispatch_lines = vec![
        Line::from(vec![
            ...
            Span::styled(
                format!(
                    "L{} {}",
                    last_ev.map(|e| e.layer_idx.saturating_sub(1)).unwrap_or(0),
    ...
    frame.render_widget(
        Paragraph::new(dispatch_lines).block(panel_block("Dispatch", t)),
        top_cols[0],
    );
    ...
    let pulse_lines = vec![
        ...
        Line::from(vec![Span::styled(
            format!("  tempo   {}ms/frame", state.tick_rate_ms),
            Style::default().fg(t.muted),
        )]),
    ];
    frame.render_widget(
        Paragraph::new(pulse_lines).block(panel_block("Pulse", t)),
        top_cols[1],
    );
    ...
    let reg_lines = vec![
        Line::from(vec![
            ...
            Span::styled(" ", Style::default()),
            badge(
                &format!("peak {:.1}", state.peak_tok_per_sec),
                t.bg,
                t.accent_alt,
            ),
        ]),
        Line::from(vec![
            badge(
                &format!("{} proven", state.total_tokens_proven),
                t.bg,
                t.muted,
            ),
            Span::styled(" ", Style::default()),
            badge(
                &format!("{} on-chain", state.verification_count),
                t.bg,
                t.accent_soft,
            ),
        ]),
    ...
    frame.render_widget(
        Paragraph::new(reg_lines).block(panel_block("Registers", t)),
        top_cols[2],
    );
    ...
    let tp_points: Vec<(f64, f64)> = state
        .throughput_history
        .iter()
        .enumerate()
        .map(|(i, &v)| (i as f64, v as f64))
        .collect();
    let gpu_points: Vec<(f64, f64)> = state
        .gpu_util_history
        .iter()
        .enumerate()
    ...
    let max_ms = state
        ...
        .max()
        .unwrap_or(1);
    let layer_items: Vec<ListItem> = state
        .layer_events
        .iter()
        .take(3)
        .map(|e| {
            ...
            let type_color = match e.layer_type.as_str() {
                "MatMul" => t.accent,
                "RMSNorm" => t.accent_soft,
                "Add" => t.muted,
                _ => t.text,
            };
            ListItem::new(Line::from(vec![
                Span::styled(
                    format!("  {:<10}", e.layer_type),
                    Style::default().fg(type_color),
                ),
                Span::styled(
                    format!("{:>4}ms ", e.time_ms),
                    Style::default().fg(t.accent_alt),
                ),
        ...
        .collect();
    frame.render_widget(
        List::new(layer_items).block(panel_block("Layer Spotlight", t)),
        right_rows[0],
    );

    // Live trace
    let trace_items: Vec<ListItem> = state
        .layer_events
        .iter()
        .take(10)
        .map(|e| {
        ...
        .collect();
    frame.render_widget(
        List::new(trace_items).block(panel_block("Live Trace", t)),
        right_rows[1],
    );
    ...
    frame.render_widget(
        Paragraph::new(vec![
            ...
            Line::from(vec![
                Span::styled(
                    format!("  {:.0}% util", state.gpu_util_pct),
                    Style::default()
                        .fg(t.accent_alt)
                        .add_modifier(Modifier::BOLD),
                ),
                Span::styled(
                    format!("  {:.0}°C", state.gpu_temp_c),
                    Style::default().fg(t.muted),
                ),
            ]),
        ])
    ...
    frame.render_widget(
        Paragraph::new(vec![
            ...
            Line::from(vec![Span::styled(
                format!(
                    "  {}ms/layer",
                    state.layer_time_history.last().unwrap_or(&0)
                ),
                Style::default()
                    .fg(t.accent_soft)
                    .add_modifier(Modifier::BOLD),
    ...
    let gpu_lines = vec![
        ...
        Line::from(vec![
            Span::styled(
                format!(
                    "  {:.0}GB / {:.0}GB",
                    state.gpu_memory_gb as f64 * gpu_mem_ratio,
                    state.gpu_memory_gb
                ),
                Style::default().fg(t.text),
            ),
            Span::styled(
                format!("  {}", state.gpu_name),
                Style::default().fg(t.muted),
            ),
        ]),
        Line::from(vec![
    ...
    frame.render_widget(
        Paragraph::new(gpu_lines).block(panel_block("GPU Memory", t)),
        bot_cols[0],
    );
    ...
    let proof_lines = vec![
        Line::from(vec![
            ...
            Span::styled(
                format!("{} layers", state.proving_total_layers),
                Style::default().fg(t.text),
            ),
            Span::styled(
                format!("  {} matmuls", state.proving_total_matmuls),
                Style::default().fg(t.text),
            ),
        ]),
        Line::from(vec![
            ...
            Span::styled(
                format!("{} felts", state.stark_felts.unwrap_or(0)),
                Style::default()
                    .fg(t.accent_alt)
                    .add_modifier(Modifier::BOLD),
            ),
            Span::styled(
                format!("  {:.1}s", state.stark_time_secs.unwrap_or(0.0)),
                Style::default().fg(t.text),
            ),
        ]),
        Line::from(vec![
            Span::styled("  on-chain   ", Style::default().fg(t.muted)),
            badge(
                &format!("{} verified", state.verification_count),
                t.bg,
                t.success,
            ),
            Span::styled(
                format!("  {}", state.network),
                Style::default().fg(t.accent_soft),
            ),
        ]),
    ];
    frame.render_widget(
        Paragraph::new(proof_lines).block(panel_block("Proof Artifact", t)),
        bot_cols[1],
    );
}
...
fn render_prove(frame: &mut Frame, area: Rect, state: &InteractiveDashState, t: &Theme) {
    ...
    frame.render_widget(
        Paragraph::new(gkr_lines).block(panel_block("GKR Proof", t)),
        rows[0],
    );
    ...
    let stark_lines = if let Some(time) = state.stark_time_secs {
        vec![
            ...
            Line::from(Span::styled(
                "  GKR (~46K felts) → recursive STARK (~950 felts) → Starknet TX",
                Style::default().fg(t.muted),
            )),
        ]
    } else {
        vec![Line::from(vec![
            Span::styled(
                format!("  {} ", pulse(state.tick)),
                Style::default().fg(t.accent_alt),
            ),
            Span::styled(
                if state.proving_active {
                    "Compressing GKR proof..."
                } else {
                    "Awaiting GKR completion"
                },
                Style::default().fg(t.muted),
            ),
        ])]
    ...
    frame.render_widget(
        Paragraph::new(stark_lines).block(panel_block("Recursive STARK", t)),
        rows[1],
    );
    ...
    frame.render_widget(
        Paragraph::new(vec![
            ...
            Line::from(vec![
                Span::styled(
                    format!("  {}ms", state.matmul_time_history.last().unwrap_or(&0)),
                    Style::default().fg(t.accent).add_modifier(Modifier::BOLD),
                ),
    ...
    frame.render_widget(
        Paragraph::new(vec![
            ...
            Line::from(vec![Span::styled(
                format!("  {} perms", state.poseidon_perms),
                Style::default()
                    .fg(t.accent_alt)
                    .add_modifier(Modifier::BOLD),
            )]),
    ...
    frame.render_widget(
        Paragraph::new(vec![
            ...
            Line::from(vec![Span::styled(
                format!(
                    "  {}/{} rounds",
                    state.sumcheck_round, state.sumcheck_total_rounds
                ),
                Style::default()
                    .fg(t.accent_soft)
                    .add_modifier(Modifier::BOLD),
    ...
    // Matmul timing chart
    let mt_points: Vec<(f64, f64)> = state
        .matmul_time_history
        .iter()
        .enumerate()
        .map(|(i, &v)| (i as f64, v as f64))
        .collect();
    let pos_points: Vec<(f64, f64)> = state
        .poseidon_history
        .iter()
        .enumerate()
        .map(|(i, &v)| (i as f64, v as f64 / 3.0))
        .collect();
    let mt_max = state
        .matmul_time_history
        .iter()
    ...
    .y_axis(Axis::default().bounds([0.0, mt_max]).labels(vec![
        Span::styled("0", Style::default().fg(t.muted)),
        Span::styled(format!("{:.0}", mt_max), Style::default().fg(t.muted)),
    ]));
    ...
    let items: Vec<ListItem> = state
        ...
        .map(|e| {
            ...
            let tc = match e.layer_type.as_str() {
                "MatMul" => t.accent,
                "RMSNorm" => t.accent_soft,
                _ => t.muted,
            };
        ...
        .collect();
    frame.render_widget(
        List::new(items).block(panel_block("Live Proof Trace", t)),
        prove_bot[1],
    );
}
...
fn render_chat(frame: &mut Frame, area: Rect, state: &InteractiveDashState, t: &Theme) {
    ...
    let items: Vec<ListItem> = state
        ...
        .map(|m| {
            ...
            if let Some(ref st) = m.proof_status {
                let sc = match st.as_str() {
                    "verified" => t.success,
                    "proving" => t.accent_alt,
                    _ => t.danger,
                };
                spans.push(Span::styled(format!("  [{}]", st), Style::default().fg(sc)));
                if st == "proving" {
                    spans.push(Span::styled(
                        format!(" {} {:.0}s", pulse(state.tick), state.proving_elapsed_secs),
                        Style::default().fg(t.accent_alt),
                    ));
                }
            }
    ...
    let all_items = if state.chat_messages.is_empty() {
        empty_msg
    } else {
        items
    };
    frame.render_widget(
        List::new(all_items).block(panel_block("Verifiable Chat", t)),
        rows[0],
    );
    ...
    .block(
        Block::default()
            ...
            .title(Span::styled(
                " Enter send · Esc cancel · proof runs in background ",
                Style::default().fg(t.muted),
            ))
            .style(Style::default().bg(t.panel)),
    );
...
fn render_onchain(frame: &mut Frame, area: Rect, state: &InteractiveDashState, t: &Theme) {
    ...
    let cs = if state.contract.len() > 40 {
        format!(
            "{}...{}",
            &state.contract[..20],
            &state.contract[state.contract.len() - 8..]
        )
    } else {
    ...
    let info = vec![
        Line::from(vec![
            badge("CONTRACT", t.bg, t.accent_soft),
            Span::styled(format!("  {}", cs), Style::default().fg(t.text)),
        ]),
        Line::from(vec![
            badge(&state.network, t.bg, t.border),
            Span::styled(
                format!("  {} verifications", state.verification_count),
                Style::default().fg(t.accent).add_modifier(Modifier::BOLD),
            ),
        ]),
        Line::from(vec![
            Span::styled("  STARK: ", Style::default().fg(t.muted)),
            Span::styled(
                format!("{:.1}s compression", state.stark_time_secs.unwrap_or(0.0)),
                Style::default().fg(t.accent_alt),
            ),
            Span::styled(
                format!("  {} felts/proof", state.stark_felts.unwrap_or(0)),
                Style::default().fg(t.text),
            ),
        ]),
    ];
    frame.render_widget(
        Paragraph::new(info).block(panel_block("Starknet Verification", t)),
        rows[0],
    );
    ...
    let items: Vec<ListItem> = state
        ...
        .rev()
        .map(|tx| {
            let short = if tx.tx_hash.len() > 24 {
                &tx.tx_hash[..24]
            } else {
                &tx.tx_hash
            };
        ...
        .collect();
    frame.render_widget(
        List::new(items).block(panel_block("Verified Proofs", t)),
        rows[1],
    );
}
...
fn render_footer(frame: &mut Frame, area: Rect, state: &InteractiveDashState, t: &Theme) {
    let sc = if state.proving_active {
        t.accent_alt
    } else {
        t.success
    };
    let st = if state.proving_active {
        "PROVING"
    } else {
        "READY"
    };
    let line = Line::from(vec![
        ...
        Span::styled("+/-", Style::default().fg(t.accent)),
        Span::styled(" speed", Style::default().fg(t.muted)),
    ]);
...
fn render_model(frame: &mut Frame, area: Rect, state: &InteractiveDashState, t: &Theme) {
    ...
    let arch_lines = vec![
        ...
        Line::from(vec![
            ...
            Span::styled(
                format!("{}", state.model_hidden_size),
                Style::default().fg(t.text).add_modifier(Modifier::BOLD),
            ),
            Span::styled("  heads  ", Style::default().fg(t.muted)),
            Span::styled(
                format!("{}", state.model_num_heads),
                Style::default().fg(t.text).add_modifier(Modifier::BOLD),
            ),
        ]),
        Line::from(vec![
            ...
            Span::styled(
                format!("{}", state.model_ff_size),
                Style::default().fg(t.text).add_modifier(Modifier::BOLD),
            ),
            Span::styled("  vocab  ", Style::default().fg(t.muted)),
            Span::styled(
                format!("{}", state.model_vocab_size),
                Style::default().fg(t.text).add_modifier(Modifier::BOLD),
            ),
        ]),
        Line::from(vec![
            ...
            Span::styled(
                format!("{}", model.map(|m| m.layers).unwrap_or(0)),
                Style::default().fg(t.accent).add_modifier(Modifier::BOLD),
            ),
            Span::styled("  matmuls  ", Style::default().fg(t.muted)),
            Span::styled(
                format!("{}", model.map(|m| m.matmuls).unwrap_or(0)),
                Style::default().fg(t.accent).add_modifier(Modifier::BOLD),
            ),
        ]),
        Line::from(vec![
            ...
            Span::styled(
                "RMSNorm → Q/K/V → O → RMSNorm → gate×up → down → Add",
                Style::default().fg(t.accent_soft),
            ),
        ]),
    ];
    frame.render_widget(
        Paragraph::new(arch_lines).block(panel_block("Architecture", t)),
        left_rows[0],
    );
    ...
    let weight_lines = vec![
        Line::from(vec![
            ...
            Span::styled(
                state.weight_total_params.clone(),
                Style::default().fg(t.text).add_modifier(Modifier::BOLD),
            ),
            Span::styled(
                format!(
                    "  ({:.1} GB, {} shards)",
                    state.weight_size_gb, state.weight_shards
                ),
                Style::default().fg(t.muted),
            ),
        ]),
        ...
        Line::from(vec![
            Span::styled("  cache      ", Style::default().fg(t.muted)),
            badge(
                &state.weight_cache_status.to_uppercase(),
                t.bg,
                if state.weight_cache_status == "warm" {
                    t.success
                } else {
                    t.accent_alt
                },
            ...
            Span::styled(
                "  Poseidon Merkle roots cached to disk",
                Style::default().fg(t.muted),
            ),
        ]),
    ];
    frame.render_widget(
        Paragraph::new(weight_lines).block(panel_block("Weights", t)),
        left_rows[1],
    );
    ...
    let pos_lines = vec![
        ...
        Line::from(vec![
            Span::styled(
                format!("  {} perms total", state.poseidon_perms),
                Style::default()
                    .fg(t.accent_alt)
                    .add_modifier(Modifier::BOLD),
            ),
    ...
    frame.render_widget(
        Paragraph::new(pos_lines).block(panel_block("Poseidon Hashing", t)),
        left_rows[2],
    );
    ...
    let gpu_lines = vec![
        Line::from(vec![badge(&state.gpu_name, t.bg, t.accent_alt)]),
        gauge_line("VRAM", gpu_mem_ratio, t),
        Line::from(vec![
            Span::styled(
                format!(
                    "  {:.1}GB / {:.0}GB  ",
                    state.gpu_mem_used_gb, state.gpu_memory_gb
                ),
                Style::default().fg(t.text),
            ),
            Span::styled(
                format!("{:.0}W  {:.0}°C", state.gpu_power_w, state.gpu_temp_c),
                Style::default().fg(t.muted),
            ),
        ]),
        Line::from(Span::styled(
    ...
    frame.render_widget(
        Paragraph::new(gpu_lines).block(panel_block("GPU Detail", t)),
        right_rows[0],
    );

    // Sumcheck detail
    let sc_ratio = if state.sumcheck_total_rounds > 0 {
        state.sumcheck_round as f64 / state.sumcheck_total_rounds as f64
    } else {
        0.0
    };
    ...
    let sc_lines = vec![
        ...
        Line::from(vec![
            Span::styled(
                format!("  {}/{}", state.sumcheck_round, state.sumcheck_total_rounds),
                Style::default().fg(t.text),
            ),
    ...
    frame.render_widget(
        Paragraph::new(sc_lines).block(panel_block("Sumcheck Rounds", t)),
        right_rows[1],
    );

    // Forward pass detail
    let fwd_ratio = if state.fwd_node_total > 0 {
        state.fwd_node_current as f64 / state.fwd_node_total as f64
    } else {
        0.0
    };
    ...
    let fwd_lines = vec![
        ...
        Line::from(vec![
            Span::styled(
                format!("  {}/{}", state.fwd_node_current, state.fwd_node_total),
                Style::default().fg(t.text),
            ),
            Span::styled(
                format!("  {:.1}s elapsed", state.fwd_elapsed_secs),
                Style::default().fg(t.accent_alt),
            ),
        ]),
        Line::from(Span::styled(
    ...
    frame.render_widget(
        Paragraph::new(fwd_lines).block(panel_block("Forward Pass", t)),
        right_rows[2],
    );
}
