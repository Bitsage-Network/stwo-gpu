...
async fn chat_completions(
    ...
    let (text, trust_str, io_hex, num_tokens) = if let Some(ref local) = state.local_provider {
        ...
        if model_lower == local.model_name.to_lowercase()
            || model_lower == "local"
            || state.anthropic_provider.is_none()
                && state.openai_provider.is_none()
                && state.upstream_provider.is_none()
        {
            let local = Arc::clone(local);
            let max_tok = req.max_tokens.unwrap_or(1) as usize;
            let result = tokio::task::spawn_blocking(move || {
                local
                    .generate_with_proof(&prompt, max_tok, |_, _, _| {})
                    .map_err(|e| format!("{e}"))
            })
            .await
            ...
            *state.last_proof_meta.write().await = pm.clone();
            ...
            if let Some(ref anthropic) = state.anthropic_provider {
                let (res, att) = anthropic
                    .chat(&messages, req.max_tokens)
                    .await
                ...
                state
                    ...
                    .write()
                    .await
                    .insert(att.attestation_id.clone(), att);
                (
                    res.text,
                    trust_model_str(&res.trust_model),
                    res.io_commitment.map(|c| format!("0x{:x}", c)),
                    res.num_tokens,
                )
            } else {
                return Err(api_error(
            ...
            || model_lower.starts_with("o3")
        {
            ...
            (
                result.text,
                trust_model_str(&result.trust_model),
                result.io_commitment.map(|c| format!("0x{:x}", c)),
                result.num_tokens,
            )
        } else {
            return Err(api_error(
                StatusCode::BAD_REQUEST,
                &format!("Model '{}' not available", req.model),
            ));
        }
    } else if model_lower.starts_with("claude") {
        if let Some(ref anthropic) = state.anthropic_provider {
...
async fn chat_completions_stream(
    ...
    let local = state.local_provider.as_ref().ok_or_else(|| {
        api_error(
            StatusCode::SERVICE_UNAVAILABLE,
            "no local provider (streaming requires local model)",
        )
    })?;
    let local = Arc::clone(local);
...
async fn list_models(State(state): State<Arc<VmState>>) -> Json<serde_json::Value> {
...
async fn prove_batch(
    ...
    let result = tokio::task::spawn_blocking(move || {
        local.prove_batch(&token_ids).map_err(|e| format!("{e}"))
    })
    .await
    .map_err(|e| api_error(StatusCode::INTERNAL_SERVER_ERROR, &format!("join: {e}")))?
    .map_err(|e| api_error(StatusCode::INTERNAL_SERVER_ERROR, &e))?;
...
async fn list_sessions(State(state): State<Arc<VmState>>) -> Json<serde_json::Value> {
...
async fn main() {
    ...
    let local_provider = std::env::var("OBELYSK_MODEL_DIR").ok().and_then(|dir| {
    ...
    let upstream_provider = std::env::var("UPSTREAM_URL").ok().map(|url| {
        let model = std::env::var("UPSTREAM_MODEL").unwrap_or_else(|_| "default".into());
        let provider_name =
            std::env::var("UPSTREAM_PROVIDER").unwrap_or_else(|_| "upstream".into());
        ...
        tokio::spawn(async move {
            ...
            loop {
                ...
                sessions.retain(|_, sess| {
                    sess.last_accessed.elapsed() < std::time::Duration::from_secs(300)
                });
                let evicted = before - sessions.len();
                if evicted > 0 {
                    eprintln!(
                        "[vm] evicted {} expired sessions ({} active)",
                        evicted,
                        sessions.len()
                    );
                }
    ...
    loop {
        ...
        if state.proving_active {
            if let Some(start) = proving_start {
                ...
                } else if state.proving_elapsed_secs < 55.0 {
                    state.proving_phase = "gkr".into();
                } else {
                    state.proving_phase = "stark".into();
                }
                ...
                let tps = if state.proving_elapsed_secs > 0.0 {
                    (1.0 / state.proving_elapsed_secs * 100.0) as u64
                } else {
                    0
                };
                ...
                push_limited(
                    &mut state.layer_time_history,
                    180 + ((tick * 7) % 200) as u64,
                );
                push_limited(
                    &mut state.matmul_time_history,
                    200 + ((tick * 11) % 150) as u64,
                );
                push_limited(&mut state.fwd_history, 20 + ((tick * 3) % 40) as u64);
                push_limited(
                    &mut state.poseidon_history,
                    300 + ((tick * 13) % 600) as u64,
                );
                ...
                if tick % 8 == 0 && state.proving_layer > 0 {
                    ...
                    if state.layer_events.len() > 20 {
                        state.layer_events.pop();
                    }
                }
            }
        } else {
        ...
        while let Ok(response) = chat_rx.try_recv() {
            ...
            if let Ok(data) = serde_json::from_str::<serde_json::Value>(&response) {
                ...
                let proof_status = if tx_hash.is_some() {
                    "verified"
                } else {
                    "proved"
                };
            ...
            match event::read() {
                Ok(Event::Key(key)) => {
                    ...
                    match key.code {
                        ...
                        KeyCode::Char('1') if state.mode != Mode::Chat => {
                        ...
                        KeyCode::Char('4') if state.mode != Mode::Chat => {
                            state.mode = Mode::OnChain
                        }
                        ...
                        KeyCode::Char('t') if state.mode != Mode::Chat => {
                            state.theme_idx =
                                (state.theme_idx + 1) % obelyzk::tui::interactive::THEMES.len();
                        }
                        ...
                        KeyCode::Tab if state.mode != Mode::Chat => {
                            if !state.models.is_empty() {
                                state.selected_model =
                                    (state.selected_model + 1) % state.models.len();
                            }
                        }
                        ...
                        KeyCode::Backspace
                            if state.mode == Mode::Chat && state.input_cursor > 0 =>
                        {
                            state.input_cursor -= 1;
                        ...
                        KeyCode::Enter
                            if state.mode == Mode::Chat && !state.input_buffer.is_empty() =>
                        {
...
async fn run_benchmark(args: &[String]) {
    let model_dir = std::env::var("OBELYSK_MODEL_DIR").unwrap_or_else(|_| {
    ...
    let provider = LocalProvider::load(&PathBuf::from(&model_dir), None).unwrap_or_else(|e| {
        eprintln!("Error: {e}");
        std::process::exit(1);
    });
    ...
    println!(
        "  \x1b[90mMODEL\x1b[0m    \x1b[97;1m{}\x1b[0m",
        provider.model_name
    );
    ...
    let result = tokio::task::spawn_blocking(move || provider.prove_batch(&token_ids))
        .await
        .unwrap();
    ...
    match result {
        Ok((_output, proof, prove_time_ms)) => {
            ...
            println!("  \x1b[92m════════════════════════════════════════\x1b[0m");
            println!(
                "  \x1b[90mTokens:\x1b[0m        \x1b[97;1m{}\x1b[0m",
                num_tokens
            );
            println!(
                "  \x1b[90mTotal time:\x1b[0m    \x1b[97;1m{:.1}s\x1b[0m",
                elapsed
            );
            println!(
                "  \x1b[90mThroughput:\x1b[0m    \x1b[92;1m{:.1} tok/s\x1b[0m",
                tok_per_sec
            );
            println!(
                "  \x1b[90mPer token:\x1b[0m     {:.1}ms",
                (elapsed * 1000.0) / num_tokens as f64
            );
            println!(
                "  \x1b[90mIO commitment:\x1b[0m \x1b[35m0x{:x}\x1b[0m",
                proof.io_commitment
            );
            println!(
                "  \x1b[90mGPU:\x1b[0m           {}",
                if obelyzk::backend::gpu_is_available() {
...
async fn run_chat_mode(args: &[String]) {
    ...
    // Parse --model flag
    let mut model =
        std::env::var("OBELYZK_MODEL").unwrap_or_else(|_| "claude-sonnet-4-20250514".into());
    let mut i = 0;
    while i < args.len() {
    ...
    let is_openai = model_lower.starts_with("gpt")
        || model_lower.starts_with("o1")
        || model_lower.starts_with("o3");
    ...
    let trust_label = if is_anthropic || is_openai {
        "TLS attestation"
    } else {
        "ZK proof"
    };
            ...
            match gen_result {
                Ok((text, token_ids)) => {
                    ...
                    let tok_per_sec = if gen_ms > 0 {
                        n_tokens as f64 / (gen_ms as f64 / 1000.0)
                    } else {
                        0.0
                    };
        ...
        if let Some((text, commitment, att_id, latency_ms)) = result {
            ...
            let commit_short = if commitment.len() > 18 {
                format!(
                    "{}…{}",
                    &commitment[..10],
                    &commitment[commitment.len() - 6..]
                )
            } else {
                commitment.clone()
            };
            let att_short = if att_id.len() > 16 {
                &att_id[..16]
            } else {
                &att_id
            };
