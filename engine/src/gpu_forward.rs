...
impl GpuForwardExecutor {
    ...
    pub fn download(&self, t: &GpuTensor) -> Result<M31Matrix, GpuForwardError> {
        ...
        self.device
            .dtoh_sync_copy_into(&t.data, &mut buf)
            .map_err(|e| GpuForwardError::MemoryError(format!("download: {e:?}")))?;
        Ok(M31Matrix {
            rows: t.rows,
            cols: t.cols,
            data: buf.into_iter().map(M31::from).collect(),
        })
    }
    ...
    pub fn matmul(&self, a: &GpuTensor, b: &GpuTensor) -> Result<GpuTensor, GpuForwardError> {
        ...
        if k != b.rows {
            return Err(GpuForwardError::KernelError(format!(
                "matmul: a.cols={k} != b.rows={}",
                b.rows
            )));
        }
    ...
    pub fn upload_weights(
        ...
        eprintln!(
            "[gpu-forward] Pre-uploaded {} weight matrices to GPU",
            gpu_weights.len()
        );
        Ok(gpu_weights)
    }
