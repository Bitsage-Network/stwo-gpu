...
use stwo_verifier_core::fields::qm31::{QM31, QM31One, QM31Zero};
...
pub fn cube_252_at_point(
    ...
    loop {
        ...
        quotients.append(*q1s[i]);
        i += 1;
    }

    // x_sq * x = x_cubed (55 constraints)
    let q2 = verify_mul_252_at_point(
        x_sq, x, x_cubed, k_cube_limbs, carries_cube, p_limbs, is_active,
    );
    let q2s = q2.span();
    i = 0;
    loop {
...
pub fn mds_at_point(
    ...
    mds_row_at_point(
        a, b, c, out0, carries0, k0, @coeffs0, p_limbs, base, is_active, ref quotients,
    );

    // MDS row 1: a - b + c
    let coeffs1: Array<QM31> = array![one, neg_one, one];
    mds_row_at_point(
        a, b, c, out1, carries1, k1, @coeffs1, p_limbs, base, is_active, ref quotients,
    );

    // MDS row 2: a + b - 2c
    let coeffs2: Array<QM31> = array![one, one, neg_two];
    mds_row_at_point(
        a, b, c, out2, carries2, k2, @coeffs2, p_limbs, base, is_active, ref quotients,
    );
...
fn mds_row_at_point(
    ...
    loop {
        ...
        let p_j: QM31 = if j < 28 {
            qm31_from_u32(*p_limbs.at(j))
        } else {
            zero
        };
        let carry_in: QM31 = if j == 0 {
        ...
        let carry_out: QM31 = if j < 29 {
            *carries.at(j)
        } else {
            zero
        };

        let a_j: QM31 = if j < 28 {
            *a.at(j)
        } else {
            zero
        };
        let b_j: QM31 = if j < 28 {
            *b.at(j)
        } else {
            zero
        };
...
/// Return 2^exp embedded as a QM31 real component.
fn qm31_pow2(exp: u32) -> QM31 {
    let mut v: u32 = 1;
    let mut i: u32 = 0;
    loop {
        if i >= exp {
...
///
/// Reads 1281 columns from trace_vals starting at col_offset (= 56 for chain+hades).
/// Returns an array of constraint quotients.
///
/// Column layout (1281 columns, matching Rust air.rs):
...
///   post_sbox:       3 × 28 = 84
///   mds_result:      3 × 28 = 84
///   mds_carries+k:   3 × 30 = 90
///   shifted_next:    3 × 28 = 84
///   selectors:       5
///   input carry:     28 + 28
///   repack:          50
///   TOTAL:           1281
/// Returns the array of Hades constraint quotients.
...
pub fn evaluate_hades_constraints_array(
    trace_vals: stwo_verifier_core::ColumnSpan<Span<QM31>>, col_offset: u32,
) -> Array<QM31> {
    ...
    loop {
        ...
        loop {
            ...
            loop {
                ...
                carries.append(extract_single_val(trace_vals, pos));
                pos += 1;
                j += 1;
            }
            ...
            loop {
                ...
                k_vals.append(extract_single_val(trace_vals, pos));
                pos += 1;
                j += 1;
            }
    ...
    loop {
        ...
        mds_k.append(extract_single_val(trace_vals, pos));
        pos += 1;
        elem += 1;
    }
    ...
    // Selectors
    let is_full_round = extract_single_val(trace_vals, pos);
    pos += 1;
    let is_real = extract_single_val(trace_vals, pos);
    pos += 1;
    let is_chain_round = extract_single_val(trace_vals, pos);
    pos += 1;
    let is_first_round = extract_single_val(trace_vals, pos);
    pos += 1;
    let is_last_round = extract_single_val(trace_vals, pos);
    pos += 1;

    let mut provider_input: Array<QM31> = array![];
    let mut j: u32 = 0;
    loop {
        if j >= 28 {
            break;
        }
        provider_input.append(extract_single_val(trace_vals, pos));
        pos += 1;
        j += 1;
    }

    let mut shifted_provider_input: Array<QM31> = array![];
    j = 0;
    loop {
        if j >= 28 {
            break;
        }
        shifted_provider_input.append(extract_single_val(trace_vals, pos));
        pos += 1;
        j += 1;
    }

    // input_digest_28bit: 9
    let mut input_digest_28bit: Array<QM31> = array![];
    j = 0;
    loop {
        if j >= 9 {
            break;
        }
        input_digest_28bit.append(extract_single_val(trace_vals, pos));
        pos += 1;
        j += 1;
    }

    // output_digest_28bit: 9
    let mut output_digest_28bit: Array<QM31> = array![];
    j = 0;
    loop {
        if j >= 9 {
            break;
        }
        output_digest_28bit.append(extract_single_val(trace_vals, pos));
        pos += 1;
        j += 1;
    }

    // split witnesses: 4 × 8
    let mut split_lo_in: Array<QM31> = array![];
    j = 0;
    loop {
        if j >= 8 {
            break;
        }
        split_lo_in.append(extract_single_val(trace_vals, pos));
        pos += 1;
        j += 1;
    }
    let mut split_hi_in: Array<QM31> = array![];
    j = 0;
    loop {
        if j >= 8 {
            break;
        }
        split_hi_in.append(extract_single_val(trace_vals, pos));
        pos += 1;
        j += 1;
    }
    let mut split_lo_out: Array<QM31> = array![];
    j = 0;
    loop {
        if j >= 8 {
            break;
        }
        split_lo_out.append(extract_single_val(trace_vals, pos));
        pos += 1;
        j += 1;
    }
    let mut split_hi_out: Array<QM31> = array![];
    j = 0;
    loop {
        if j >= 8 {
            break;
        }
        split_hi_out.append(extract_single_val(trace_vals, pos));
        pos += 1;
        j += 1;
    }
    ...
    quotients.append(is_first_round * (is_first_round - one));
    quotients.append(is_last_round * (is_last_round - one));
    ...
    loop {
        ...
        loop {
            ...
            let expected = is_full_round * *cr_e[j] + (one - is_full_round) * *si_e[j];
    ...
    // Provider input binding: the LogUp provider key's input digest must equal
    // state_before[0] on the first round and must be carried unchanged through
    // the block until the last-round provider row.
    let sb_s = @state_before;
    let sb0: Span<QM31> = sb_s.at(0).span();
    let pi: Span<QM31> = provider_input.span();
    let spi: Span<QM31> = shifted_provider_input.span();
    j = 0;
    loop {
        if j >= 28 {
            break;
        }
        quotients.append(is_first_round * (*pi[j] - *sb0[j]));
        quotients.append(is_chain_round * (*pi[j] - *spi[j]));
        j += 1;
    }

    // Repack verification for future Hades LogUp provider keys. On last-round
    // rows, the 28-bit input/output key columns must be the packed form of the
    // carried input digest and verified Hades output state.
    let in28: Span<QM31> = input_digest_28bit.span();
    let out28: Span<QM31> = output_digest_28bit.span();
    let lo_in: Span<QM31> = split_lo_in.span();
    let hi_in: Span<QM31> = split_hi_in.span();
    let lo_out: Span<QM31> = split_lo_out.span();
    let hi_out: Span<QM31> = split_hi_out.span();
    let out9: Span<QM31> = mr_s.at(0).span();
    ...
    loop {
        ...
        let lo_bits = j + 1;
        let split_limb = 3 * (j + 1);
        let two_pow_lo = qm31_pow2(lo_bits);
        quotients.append(is_last_round * (*pi[split_limb] - *lo_in[j] - *hi_in[j] * two_pow_lo));
        quotients
            .append(is_last_round * (*out9[split_limb] - *lo_out[j] - *hi_out[j] * two_pow_lo));
        j += 1;
    }

    // b[0] = a[0] + a[1]*2^9 + a[2]*2^18 + lo(a[3])*2^27
    let expected_in_b0 = *pi[0]
        + *pi[1] * qm31_pow2(9)
        + *pi[2] * qm31_pow2(18)
        + *lo_in[0] * qm31_pow2(27);
    quotients.append(is_last_round * (*in28[0] - expected_in_b0));

    let expected_out_b0 = *out9[0]
        + *out9[1] * qm31_pow2(9)
        + *out9[2] * qm31_pow2(18)
        + *lo_out[0] * qm31_pow2(27);
    quotients.append(is_last_round * (*out28[0] - expected_out_b0));
    ...
    loop {
        ...
        let hi_bits_prev = 9 - j;
        let j_base = 3 * j + 1;
        let shift_lo = 27 - j;

        let expected_in = *hi_in[j
            - 1]
            + *pi[j_base] * qm31_pow2(hi_bits_prev)
            + *pi[j_base
            + 1] * qm31_pow2(hi_bits_prev + 9)
            + *lo_in[j] * qm31_pow2(shift_lo);
        quotients.append(is_last_round * (*in28[j] - expected_in));

        let expected_out = *hi_out[j
            - 1]
            + *out9[j_base] * qm31_pow2(hi_bits_prev)
            + *out9[j_base
            + 1] * qm31_pow2(hi_bits_prev + 9)
            + *lo_out[j] * qm31_pow2(shift_lo);
        quotients.append(is_last_round * (*out28[j] - expected_out));

        j += 1;
    }

    // b[8] = hi(a[24]) + a[25]*2 + a[26]*2^10 + a[27]*2^19
    let expected_in_b8 = *hi_in[7]
        + *pi[25] * qm31_pow2(1)
        + *pi[26] * qm31_pow2(10)
        + *pi[27] * qm31_pow2(19);
    quotients.append(is_last_round * (*in28[8] - expected_in_b8));

    let expected_out_b8 = *hi_out[7]
        + *out9[25] * qm31_pow2(1)
        + *out9[26] * qm31_pow2(10)
        + *out9[27] * qm31_pow2(19);
    quotients.append(is_last_round * (*out28[8] - expected_out_b8));
